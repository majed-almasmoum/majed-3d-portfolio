...
const hdrs  = { Authorization: `token ${TOKEN}`, Accept: ‘application/vnd.github.v3+json’, ‘Content-Type’: ‘application/json’, ‘User-Agent’: ‘3d-portfolio’ };
let works = [], sha;
const get = await fetch(url, { headers: hdrs });
if (get.ok) { const d = await get.json(); sha = d.sha; works = JSON.parse(Buffer.from(d.content, ‘base64’).toString(‘utf-8’)); }
const newWork = { …work, id: Date.now(), addedAt: new Date().toISOString() };
works.unshift(newWork);
const put = await fetch(url, { method: ‘PUT’, headers: hdrs, body: JSON.stringify({ message: `Add: ${work.name}`, content: Buffer.from(JSON.stringify(works, null, 2)).toString(‘base64’), …(sha && { sha }) }) });
